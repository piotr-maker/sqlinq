...
protected:
  ...
  small_int_v SMALLINT NOT NULL,
  int_v INTEGER NOT NULL,
  big_int_v BIGINT NOT NULL,

  float_v FLOAT NOT NULL,
  double_v REAL NOT NULL,
  decimal_v DECIMAL(8,2) NOT NULL,

  blob_v BLOB NOT NULL,
  text_v TEXT,

  bool_v BOOLEAN NOT NULL,
  ...
  (
    ...
    3.14, 2.71828, 2400.59,
    X'CAFEBABE', NULL,
    TRUE, '2023-08-08', '14:30:00', '2023-08-08 14:30:00', '2023-08-08 14:30:00'
  ),
  (
    -128, -32768, -123456, -9223372036854775808,
    -3.14, -2.71828, 1800.50,
    X'DEADBEEF', 'Text field',
    FALSE, '1999-12-31', '23:59:59', '1999-12-31 23:59:59', '1999-12-31 23:59:59'
  )
;)";
  ...
  void SetUp() override {
    ...
    backend_.stmt_init();
    backend_.stmt_prepare("DROP TABLE IF EXISTS test");
...
TEST_F(MySQLBackendTest, SelectRows) {
  TestModel model;
  TestBindData bind;
...
TEST_F(MySQLBackendTest, SelectRowsWithBindedParams) {
...
TEST_F(MySQLBackendTest, SelectTruncatedRows) {
  TestModel model;
  TestBindData bind;
  std::vector<TestModel> records;
...
TEST_F(MySQLBackendTest, InsertRows) {
  TestModel rows[] = {
      {.id = std::nullopt,
       .tiny_int_v = 12,
       .small_int_v = 32,
